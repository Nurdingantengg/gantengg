...
───────────────────────────────────────────────────────
 [\x1b[1;96m+%s] Author   : NURDIN GANTENG
 [\x1b[1;96m+%s] Github   : www.github.com/Nurdingantengg
 [\x1b[1;96m+%s] Facebook : AHMAD NURDIN TARIGAN
───────────────────────────────────────────────────────\n'''%(N,N,N,N))
...
class crack:
			...
			while True:
				...
				if pw in ['']:
					...
					def manual(xnxx=None):
						...
						if mani in ['']:
							print(' %s [%s!%s] Jangan kosong nurdin ganteng'%(N,M,N));self.manual()
						elif mani in ['1','01']:
							print(' %s '%(N))
	...
	def b_api(self,user,pwx): # Kamu jahat :v
		...
		sys.stdout.write('\r %s* %s[%scrack%s] %s/%s [OK:%s - CP:%s] %s*'%(eram,N,O,N,loop,len(self.id),len(ok),len(cp),nufi));sys.stdout.flush() # ganteng lo...
		try:
			for pw in pwx:
				pw = pw.lower()
				ses = requests.Session()
...
# JANGAN LUPA ISTIRAHAT NURDIN GANTENG # 
